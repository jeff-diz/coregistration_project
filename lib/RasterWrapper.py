...
class Raster():
    ...
    def RasterBounds(self):
        ...
        logging.debug('Raster bounds: {}'.format([ulx, lry, lrx, uly]))
    ...
    def SamplePoint(self, point):
        ...
        try:    
            ...
            logging.debug('Point not within raster bounds: {}'.format(point))
            logging.debug(e)
            point_value = None
    ...
    def SampleRandomPoints(self, n, keep_nodata=False, other=None):
        """
        Samples n random points within the Raster object.
        If, other (a different Raster object) is provided,
        sample only points within random points.
        """
        self.bounds = self.RasterBounds()
        ulx, lry, lrx, uly = self.bounds
        
        sampled_pts = []
        while len(sampled_pts) < n:
            # Create a random point within the bounds of the 
            # raster, excluding the last row and col of the
            # array (or else IndexError)
            pt = (random.uniform(lry-self.pixel_height, uly), 
                  random.uniform(ulx-self.pixel_width, lrx))
            # Sample raster at the random point
            val = self.SamplePoint(pt)
            other_val = other.SamplePoint(pt)
            if not keep_nodata:
                if val != self.nodata_val and other_val != other.nodata_val:
                    sampled_pts.append((pt, val, other_val))
            else:
                sampled_pts.append((pt, val, other_val))
